...
class PupperV3Env(PipelineEnv):
    ...
    def sample_command_foot(self, rng: jax.Array):
        # target_x_range = self._foot_target_x_range
        # target_y_range = self._foot_target_y_range
        # target_z_range = self._foot_target_z_range

        # target_x_range = (0.05, 0.25)
        # target_y_range = (rem)

        # rng, k_x, k_y, k_z, k_foot, k_zero = jax.random.split(rng, 6)

        # t_x = jax.random.uniform(k_x, (1,), minval=target_x_range[0], maxval=target_x_range[1])
        # t_y = jax.random.uniform(k_y, (1,), minval=target_y_range[0], maxval=target_y_range[1])
        # t_z = jax.random.uniform(k_z, (1,), minval=target_z_range[0], maxval=target_z_range[1])

        # foot_idx = jax.random.bernoulli(k_foot, p=0.5).astype(jp.float32)
        # zero_cmd_prob = jax.random.uniform(k_zero, (1,))
    ...
    def step(self, state: State, action: jax.Array) -> State:  # pytype: disable=signature-mismatch
        ...
        rewards_dict = {
            ...
            "reward_stand": rewards.reward_stand(
                state.info["command"],
                pipeline_state
            )
            ,
