...
controller_interface::CallbackReturn LegPoseNeuralController::on_init() {
  try {
...
controller_interface::CallbackReturn LegPoseNeuralController::on_configure(
    const rclcpp_lifecycle::State & /*previous_state*/) {
  RCLCPP_INFO(get_node()->get_logger(), "configure successful");
  return controller_interface::CallbackReturn::SUCCESS;
}

controller_interface::InterfaceConfiguration LegPoseNeuralController::command_interface_configuration()
    const {
  return controller_interface::InterfaceConfiguration{
      controller_interface::interface_configuration_type::ALL};
}

controller_interface::InterfaceConfiguration LegPoseNeuralController::state_interface_configuration()
    const {
  return controller_interface::InterfaceConfiguration{
      controller_interface::interface_configuration_type::ALL};
}

controller_interface::CallbackReturn LegPoseNeuralController::on_activate(
    const rclcpp_lifecycle::State & /*previous_state*/) {
  // Clear command buffers to ignore pre-activation commands
  rt_manip_cmd_ptr_ = 
      realtime_tools::RealtimeBuffer<std::shared_ptr<std_msgs::msg::Float32MultiArray>>(nullptr);
  ...
  manip_cmd_subscriber_ = get_node()->create_subscription<std_msgs::msg::Float32MultiArray>(
    "/manip_command", rclcpp::SystemDefaultsQoS(),
    [this](const std_msgs::msg::Float32MultiArray::SharedPtr msg) {
      rt_manip_cmd_ptr_.writeFromNonRT(msg);
    });
...
controller_interface::CallbackReturn LegPoseNeuralController::on_error(
    const rclcpp_lifecycle::State & /*previous_state*/) {
  return controller_interface::CallbackReturn::FAILURE;
}

controller_interface::CallbackReturn LegPoseNeuralController::on_deactivate(
    const rclcpp_lifecycle::State & /*previous_state*/) {
...
controller_interface::return_type LegPoseNeuralController::update(const rclcpp::Time &time,
  ...
  auto manip_cmd = rt_manip_cmd_ptr_.readFromRT();
  if (manip_cmd && manip_cmd->get()) {
      // Assuming your C++ node sends [x, y, z, leg_id]
      if (manip_cmd->get()->data.size() >= 4) {
          target_x_      = manip_cmd->get()->data.at(0);
          target_y_      = manip_cmd->get()->data.at(1);
          target_z_      = manip_cmd->get()->data.at(2);
          target_leg_id_ = manip_cmd->get()->data.at(3);
      }
  }
  ...
  try {
    ...
    // Leg Pose commands
    observation_.at(6) = (float)target_x_;
    observation_.at(7) = (float)target_y_;
    observation_.at(8) = (float)target_z_;
    observation_.at(9) = (float)target_leg_id_;
    // Orientation commands
    observation_.at(10) = (float)desired_world_z_in_body_frame_.getX();
    observation_.at(11) = (float)desired_world_z_in_body_frame_.getY();
    observation_.at(12) = (float)desired_world_z_in_body_frame_.getZ();
...
PLUGINLIB_EXPORT_CLASS(neural_controller::LegPoseNeuralController,
                       controller_interface::ControllerInterface)